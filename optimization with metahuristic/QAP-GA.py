...
New_Dist_DF = Dist.reindex(columns=Initial_Solution, index=Initial_Solution)# reindexing
New_Dist_Arr = np.array(New_Dist_DF)# transform to array
# Make a dataframe of the cost of the initial solution
Objfun1_start = pd.DataFrame(New_Dist_Arr*Flow)
Objfun1_start_Arr = np.array(Objfun1_start)# transform to array
sum_start_int = sum(sum(Objfun1_start_Arr))
print(sum_start_int)

# function of same above done one by one
def distance(array):
...
X0 = Initial_Solution[:]# for printing of initial solution
...
for i in range(int(pop)): # Shuffles the elements in the vector n times and stores them(100 random solution in each generation)
    rnd_sol_1 = rd.sample(Initial_Solution,len(X0))# randomly suffeled solution 100 time
...
Min_for_all_Generations_for_Mut_1 = np.empty((0,len(X0)+1))# +1 to keep track of fitness value
Min_for_all_Generations_for_Mut_2 = np.empty((0,len(X0)+1))

Min_for_all_Generations_for_Mut_1_1 = np.empty((0,len(X0)+2))# +2 to keep track of fitness value and at what generation solution achieve
...
for i in range(gen):
    ...
    for j in range(int(pop/2)): # range(int(pop/2))
        ...
        for i in range(2): # to select two parent 
            
            Battle_Troops = []
  #randomly select three index from solution          
            Warrior_1_index = np.random.randint(0,len(n_list))# suppose its index 2
            ...
            Warrior_1 = n_list[Warrior_1_index,:]# then this going to be a n_list[2]
            Warrior_2 = n_list[Warrior_2_index,:]
            Warrior_3 = n_list[Warrior_3_index,:]
  # three warrier to going to fight to be parent no one          
            Battle_Troops = [Warrior_1,Warrior_2,Warrior_3]
            
            
            # For Warrior #1
            Prize_Warrior_1 = distance(Warrior_1) # calculate cost function of warrier_1 from distance function
    ...
    # minimum of above will win and be the parent no one        
            ...
            Parents = np.vstack((Parents,Winner))# here is minimum no(parent)
        
        
        
        Parent_1 = Parents[0]# first parent at index 0 for one loop
        Parent_2 = Parents[1]# second parent at index 1 for second loop
        ...
        Ran_CO_1 = np.random.rand()# create random no
        
        if Ran_CO_1 < p_c: # if random no less then probability of cross over, then we perforn crossover
            
            # Choose two random numbers to crossover with their locations
            Cr_1 = np.random.randint(0,len(X0))# create two random integer
            Cr_2 = np.random.randint(0,len(X0))
 # to change gna of two parent at index cr_1, cr_2           
            while Cr_1 == Cr_2:
                Cr_2 = np.random.randint(0,len(X0))
            
            if Cr_1 < Cr_2:
            
                Cr_2 = Cr_2 + 1  #[1,3+1] since index from 0
                ...
                First_Seg_1 = Parent_1[:Cr_1] # First seg. of parent #1(not including cr_1)
        ...
        else: # If random number was above p_c(probability of crossover)
        ...
        A1 = Ran_Mut_2 #suppose 2
        A2 = Ran_Mut_3 # suppose 5
        ...
        if Ran_Mut_1 < p_m: # If probablity to mutate is less than p_m, then mutate
            if A1 < A2:
                M_Child_1_Pos_1 = Child_1[A1] # Take the index(at 2)
                M_Child_1_Pos_2 = Child_1[A2] # Take the index(at 5)
                A2 = A2+1  #[1,3+1] since index from 0
        ...
        All_in_Generation_X_1_1_temp = Mutated_Child_1[np.newaxis]# to flip yhe axis
        All_in_Generation_X_1_1 = np.column_stack((Total_Cost_Mut_1, All_in_Generation_X_1_1_temp))# stack fitness value next to mutant solution
    ...
    for i in Save_Best_in_Generation_X:
        ...
        if (Save_Best_in_Generation_X[t,:1]) <= min(Save_Best_in_Generation_X[:,:1]): # here minimum is best
    ...
    For_Plotting_the_Best = np.vstack((For_Plotting_the_Best,Final_Best_in_Generation_X))# for ploting at end
    ...
    for i in Save_Best_in_Generation_X:
        ...
        if (Save_Best_in_Generation_X[t,:1]) >= max(Save_Best_in_Generation_X[:,:1]):  # here max is worst
...
One_Final_Guy_Final = Final_Here[np.newaxis]# final guy (flip the axis)
