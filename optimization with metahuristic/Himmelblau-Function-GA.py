...
K = 3 # 3 contestent for turnament selection
pop = 160 #population(solution)
gen = 60 # generation


# x and y, 13 for x and 13 for y(two variable)
...
for i in range(pop): # Shuffle the elements in the vector n times and store them
    rd.shuffle(XY0)  #160 randomly suffeled solution
...
def objective_value(array):  
    ...
    a_Y = -6  #lower bound
    b_Y = 6  # upper bound
    l_Y = (len(array)//2) # Length of chromosome y
    
    Precision_X = (b_X-a_X)/((2**l_X)-1) #precision of x
    Precision_Y = (b_Y-a_Y)/((2**l_Y)-1)  #precision of y
...
# A=  objective_value(xy0)[1] ....1=Decoded_X...2 for Decoded_Y...3 for fitness value(objective fn)
...
One_Finel_Guy_Final = []  # track of best solution

Min_for_all_Generations_for_Mut_1 = np.empty((0,len(XY0)+1))# for child 1, fitness value and at what generation
...
for i in range(gen):
    ...
    for j in range(int(pop/2)): # Range N/2
        ...
        for i in range(2): # because we need two parent
            
            Battle_Troops = []
            # at random select 3 solution from parent
...
A = (One_Final_Guy_Final[:,1]) # objective function evaluated at One_Final_Guy_Final
